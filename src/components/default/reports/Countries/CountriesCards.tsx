...
import { IViewProps, IReportCountry } from './types';
...
function CountryCard({ elem }: ICountryCard) {
        ...
        >
          {view === 'main' && (
            <Stack sx={{ minHeight: 'inherit' }} gap='1rem'>
              <CustomChip
                label={elem?.country?.name}
                data={
                  {
                    name: elem?.country?.name,
                    image: elem?.country?.image ?? '/images/question_mark.svg',
                  } as any
                }
              />
              <CardsSlider>
                {fieldsKeys.map((key) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {key !== 'total' ? dayjs(key).format('MMMM') : key}
                      </Typography>
    ...
    >
      ...
      >
        {visibleCards.map((row) => (
          <CountryCard key={row.id} elem={row} />
        ))}
      </Box>
    </Stack>
  );
