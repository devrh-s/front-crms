...
enum TabsValues {
  ...
  VERIFICATION_ACCOUNTS = 'Account Verifications',
}
...
export default function Account({ id }: { id: number }) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await apiGetData(`accounts/${id}`);
      return response;
    },
    refetchOnWindowFocus: false,
    placeholderData: {
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_accounts_activities',
        'Account Users': 'view_account_users',
        'Pass Histories': 'view_account_pass_histories',
        'Account Verifications': 'view_account_verifications',
      };
