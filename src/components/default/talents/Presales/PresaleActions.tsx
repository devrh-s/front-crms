...
import { genders } from '@/components/default/talents/Presales/Presales';
...
import { IActionsProps, IFormInputs, IRenderInput, ITalentInput, PresaleFields } from './types';
...
export default function PresaleActions({
  ...
  const isMoved = isFromJobApplications || isFromCandidates || isFromEmployees;
  ...
  const handleErrors = (errors: any, status: number) => {
    for (const [key, value] of Object.entries(errors)) {
      const errorKey = key as PresaleFields;
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => {
      if (isFromCandidates) {
        return apiSetData(`candidates/${id}/presales`, data);
      }
      if (isFromEmployees) {
        return apiSetData(`employees/${id}/presales`, data);
      }

      return apiSetData('presales', data);
    },
    onSuccess: (result) => {
      if (result.success) {
        if (isFromCandidates) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => apiUpdateData(`presales/${id}`, data),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: [isProfile ? 'presales' : 'presales'],
        });
        hideHandler();
  ...
  const onSubmit: SubmitHandler<IFormInputs> = async (data) => {
    ...
    if (id && !isDuplicate) {
      if (isFromJobApplications) {
        ...
        createMutation.mutate({ ...sendData, candidate_id: id });
      } else if (isFromEmployees) {
        createMutation.mutate({ ...sendData, employee_id: id });
      } else {
  ...
  const getQueryKey = (
    ...
    if (isFromEmployees) return ['employees', id];
    return ['presale', id];
  };
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: getQueryKey(id, isFromJobApplications, isFromCandidates, isFromEmployees),
    queryFn: async () => {
      const response = await apiGetData(
        getEndpoint(id, isFromJobApplications, isFromCandidates, isFromEmployees)
      );
    ...
    if (isFromCandidates) {
      ...
      reset({
        ...
        talents: {
          ...
          task_templates: data.talents.task_templates?.map((item: { id: number }) => item.id) ?? [],
  ...
  }, [id, visible, isFromCandidates, isFromJobApplications, isFromEmployees]);

  const commonData = isMoved ? commonDataPresales : commonDataProps;
  ...
  function renderProfile() {
    ...
    return (
      ...
      >
        ...
        {renderInputs.map(renderInputComponent)}
  ...
  function renderInputComponent(props: IRenderInput, index: number) {
    const { type } = props;
    switch (type) {
      case 'SingleSelect':
        return (
