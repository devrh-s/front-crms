...
import PresaleActions from '../Presales/PresaleActions';
import CandidatesCards from './CandidatesCards';
import { ICandidatesFilters } from './CandidatesFilter';
import { commonDataBlocks, getCommonData } from './commonData';

const CandidateActions = dynamic(() => import('./CandidateActions'), {
  ssr: false,
});
const CandidatesFilter = dynamic(() => import('./CandidatesFilter'), {
  ssr: false,
});
...
export default function Candidates() {
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['candidates', { paginationModel, sortOptions, debouncedSearchValue, applyFilters }],
    queryFn: async () => {
      ...
      return await apiGetData(`candidates?${searchParams}`);
    },
  ...
  const columns: GridColDef<ICandidateShort>[] = useMemo(
    () => [
      {
        field: 'Actions',
        headerName: '',
        ...
        getActions: (params: { id: number; row: any }) => {
          const isTransfer = params?.row?.employee_url || params?.row?.presale_url;
          const actions = [
            <GridActionsCellItem
              key='Action Profile'
              icon={<VisibilityIcon />}
            ...
            <GridActionsCellItem
              ...
              onClick={() => handleClickCopy(`/candidates/${params.id}`)}
              label='Copy'
              showInMenu
            />,
            <GridActionsCellItem
              ...
              icon={<GroupIcon />}
              disabled={params?.row?.presale_url}
              label='To Presales'
              showInMenu
            />,
            <GridActionsCellItem
              key='Action To Employees'
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams<any, any, any, GridTreeNodeWithRender>) => {
          ...
          return links && <MoreChips data={links} propName='name' sx={sx} />;
        },
        width: 150,
      },
      {
      ...
      {view === 'table' &&
        (mdDown ? (
          ...
          <CustomTable
            ...
            multiDeletePermission={isAdmin || !!pagePermissions['delete_candidates']}
