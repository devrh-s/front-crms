...
import { IEmployeesFilters } from './EmployeesFilter';

const EmployeeActions = dynamic(() => import('./EmployeeActions'), {
  ssr: false,
});
const EmployeesFilter = dynamic(() => import('./EmployeesFilter'), {
  ssr: false,
});
...
export default function Employees() {
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['employees', { paginationModel, sortOptions, debouncedSearchValue, applyFilters }],
    queryFn: async () => {
      ...
      return await apiGetData(`employees?${searchParams}`);
    },
  ...
  const columns: GridColDef<IEmployeeShort>[] = useMemo(
    () => [
      {
        field: 'Actions',
        headerName: '',
        ...
        getActions: (params: { id: number; row: any }) => {
          const actions = [
            ...
            <GridActionsCellItem
              ...
              onClick={() => handleClickCopy(`employees/${params.id}`)}
              label='Copy'
              showInMenu
            />,
            <GridActionsCellItem
      ...
      {view === 'table' &&
        (mdDown ? (
          ...
          <CustomTable
            ...
            multiDeletePermission={isAdmin || !!pagePermissions['delete_employees']}
