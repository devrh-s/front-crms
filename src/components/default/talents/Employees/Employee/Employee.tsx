...
import Contacts from '@/components/default/talents/Employees/Employee/Contacts';
import Contents from '@/components/default/talents/Employees/Employee/Contents';
import CVs from '@/components/default/talents/Employees/Employee/CVs';
import Languages from '@/components/default/talents/Employees/Employee/Languages';
import Prices from '@/components/default/talents/Employees/Employee/Prices';
import Professions from '@/components/default/talents/Employees/Employee/Professions';
import Profile from '@/components/default/talents/Employees/Employee/Profile/Profile';
import Rates from '@/components/default/talents/Employees/Employee/Rates';
...
export default function EmployeeProfile({ id }: { id: string }) {
  ...
  const pagePermissions = useMemo(
    () => getPagePermissions('Talents', 'Employees', permissions),
    [permissions]
  );
  ...
  const {
    ...
    queryFn: async () => {
      const response = await apiGetData(`employees/${id}`);
      return response;
    },
    refetchOnWindowFocus: false,
    placeholderData: {
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Professions: 'view_employees_professions',
        'Employee Rates': 'view_employees_rates',
        'Employee Salaries': 'view_employees_salaries',
        Prices: 'view_employees_prices',
        Languages: 'view_employees_languages',
        CVS: 'view_employees_cvs',
        Contacts: 'view_employees_contacts',
        Contents: 'view_employees_contents',
        Communications: 'view_employees_communications',
        Comments: 'view_employees_comments',
      };
        ...
        <>
          ...
          {data && (
            <Box>
              ...
              <Prices id={+id} isDisplayed={activeBookmark === TabsValues.PRICE} />
