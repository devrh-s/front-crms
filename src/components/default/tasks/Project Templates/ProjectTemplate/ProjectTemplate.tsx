...
enum TabsValues {
  ...
  TASK_TEMPLATE = 'Project Templates Task Templates',
}

export default function ProjectTemplate({ id }: { id: number }) {
  ...
  const {
    ...
    queryKey: ['project-template', id ?? null],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_project_templates_activities',
        'Project Templates Task Templates': 'view_project_templates_task_templates',
      };
