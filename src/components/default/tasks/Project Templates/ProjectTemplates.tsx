...
import ProjectTemplatesCards from './ProjectTemplatesCards';
import { IProjectTemplatesFilters } from './types';
import { useGuidesStore } from '@/zustand/guidesStore';

const ProjectTemplatesActions = dynamic(() => import('./ProjectTemplatesActions'), {
  ssr: false,
});
const ProjectTemplatesFilter = dynamic(() => import('./ProjectTemplatesFilter'), {
  ssr: false,
});
...
export default function ProjectTemplates() {
  ...
  const {
    ...
    toggleFilters,
    handleSetFilters,
  } = useFilters<IProjectTemplatesFilters>();
  const {
    paginationModel,
    rowCount,
    handleRowCount,
  ...
  const { isFetching, isError, data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiGetData(`project-templates?${searchParams}`);
      return response;
    },
  ...
  const pagePermissions = useMemo(
    () => getPagePermissions('Task Manager', 'Project Templates', permissions),
    [permissions]
  );
  ...
  const { data: commonData } = useQuery({
    queryKey: ['project-templates-common'],
    ...
    () => [
        ...
        getActions: (params: { id: number; row: any }) => {
          const actions = [
            <GridActionsCellItem
              key='Action Profile'
              icon={<VisibilityIcon />}
              onClick={() => router.push(`/project-templates/${params.id}`)}
              label='Profile'
              showInMenu
            />,
            <GridActionsCellItem
              ...
              onClick={() => handleClickCopy(`/project-templates/${params.id}`)}
              label='Copy'
              showInMenu
            />,
          ];
          if (
            checkPermission({
              pagePermissions,
              permissionType: 'edit_project_templates',
              isAdmin,
            })
          ) {
            actions.push(
          ...
          if (
            checkPermission({
              pagePermissions,
              permissionType: 'delete_project_templates',
              isAdmin,
            })
          ) {
            actions.push(
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams<any, number, string>) => {
          const href = `project-templates/${params.id}`;
          return <NameLink href={href} name={params.value} sx={{ pl: 0 }} />;
        },
      },
      {
        field: 'hours',
        headerName: fields?.hours ?? 'Hours',
        display: 'flex',
        description: 'Hours',
        width: 100,
      ...
      {
        field: 'milestone_templates',
        headerName: fields?.milestone_templates ?? 'Milestone Templates',
        display: 'flex',
        cellClassName: 'chips-cell',
        width: 300,
        valueFormatter: (params: any) => {
      ...
      {view === 'table' &&
        (mdDown ? (
          ...
          <CustomTable
            ...
            availableAdd={isAdmin || !!pagePermissions['add_project_templates']}
            multiDeletePermission={isAdmin || !!pagePermissions['delete_project_templates']}
            paginationModel={paginationModel}
            handlePagination={handlePagination}
            toggleFilters={toggleFilters}
            searchValue={searchValue}
