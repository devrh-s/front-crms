...
enum TabsValues {
  ...
  PRICES = 'Profession Prices',
}

export default function Profession({ id }: { id: number }) {
  ...
  const {
    ...
    queryKey: ['profession', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`professions/${id}`);
      return response;
    },
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_professions_activities',
        'Profession Prices': 'view_profession_prices',
      };
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setEditsData({
          progressableId: profession_id,
          countEdits,
          progressableType,
        })
      );
      setGuidesData({ progressableId: profession_id, progressableType });
    }
    return () => {
        ...
        <>
          ...
          {data && (
            <Box>
              ...
              <Prices id={+id} value={activeBookmark} index={TabsValues.PRICES} />
            </Box>
          )}
        </>
      )}
