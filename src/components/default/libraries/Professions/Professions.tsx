...
const ProfessionActions = dynamic(() => import('./ProfessionActions'), {
  ssr: false,
});
...
export default function Professions() {
  ...
  const {
    ...
    toggleFilters,
    handleSetFilters,
  } = useFilters<IProfessionFilters>(false);
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['professions', { paginationModel, sortOptions, debouncedSearchValue, applyFilters }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`professions?${searchParams}`);
      return response;
    },
  ...
  const pagePermissions = useMemo(
    () => getPagePermissions('Libraries', 'Professions', permissions),
    [permissions]
  );
    ...
    () => [
        ...
        getActions: (params: { id: number; row: any }) => {
          const actions = [
            <GridActionsCellItem
              key='Action Profile'
              icon={<VisibilityIcon />}
              onClick={() => router.push(`/professions/${params.id}`)}
              label='Profile'
              showInMenu
            />,
            <GridActionsCellItem
              ...
              onClick={() => handleClickCopy(`/professions/${params.id}`)}
              label='Copy'
              showInMenu
            />,
          ];
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams<any, number, string>) => {
          const href = `professions/${params.id}`;
      ...
      },
      {
        ...
        disableColumnMenu: true,
        renderCell: (params: GridRenderCellParams<any, any, any, GridTreeNodeWithRender>) => (
          <Priority priority={params?.value?.name} color={'black'} />
        ),
      },
      {
        field: 'department',
        headerName: fields?.department ?? 'Department',
      ...
      {view === 'table' &&
        (mdDown ? (
          ...
          <CustomTable
            ...
            multiDeletePermission={isAdmin || !!pagePermissions['delete_professions']}
