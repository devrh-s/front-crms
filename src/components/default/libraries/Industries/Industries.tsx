...
import { IIndustryFilters } from './types';
import { useGuidesStore } from '@/zustand/guidesStore';

const IndustriesActions = dynamic(() => import('./IndustryActions'), {
  ssr: false,
});
const IndustriesFilter = dynamic(() => import('./IndustriesFilter'), {
  ssr: false,
});
...
export default function Industries() {
  ...
  const {
    ...
    toggleFilters,
    handleSetFilters,
  } = useFilters<IIndustryFilters>(false);
  const {
    paginationModel,
    rowCount,
    handleRowCount,
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['industries', { paginationModel, sortOptions, debouncedSearchValue, applyFilters }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`industries?${searchParams}`);
      return response;
    },
  ...
  const pagePermissions = useMemo(
    () => getPagePermissions('Libraries', 'Industries', permissions),
    [permissions]
  );
    ...
    () => [
        ...
        getActions: (params: { id: number; row: any }) => {
          const actions = [
            <GridActionsCellItem
              key='Action Profile'
              icon={<VisibilityIcon />}
              onClick={() => router.push(`/industries/${params.id}`)}
              label='Profile'
              showInMenu
            />,
            <GridActionsCellItem
              ...
              onClick={() => handleClickCopy(`/industries/${params.id}`)}
              label='Copy'
              showInMenu
            />,
          ];
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams<any, number, string>) => {
          const href = `industries/${params.id}`;
      ...
      },
      {
        ...
        headerName: fields?.sub_industries ?? 'Sub Industries',
      ...
      {view === 'table' &&
        (mdDown ? (
          ...
          <CustomTable
            ...
            multiDeletePermission={isAdmin || !!pagePermissions['delete_industries']}
