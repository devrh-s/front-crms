...
export default function Industry({ id }: { id: number }) {
  ...
  const {
    ...
    queryKey: ['industry', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`industries/${id}`);
      return response;
    },
    refetchOnWindowFocus: false,
    placeholderData: {
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_industries_activities',
      };
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setEditsData({
          progressableId: industry_id,
          countEdits,
          progressableType,
        })
      );
      setGuidesData({ progressableId: industry_id, progressableType });
    }
    return () => {
