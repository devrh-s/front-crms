...
export default function Language({ id }: { id: number }) {
  ...
  const {
    ...
    queryKey: ['language', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`languages/${id}`);
      return response;
    },
    refetchOnWindowFocus: false,
    placeholderData: {
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_languages_activities',
      };
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setEditsData({
          progressableId: language_id,
          countEdits,
          progressableType,
        })
      );
      setGuidesData({ progressableId: language_id, progressableType });
    }
    return () => {
