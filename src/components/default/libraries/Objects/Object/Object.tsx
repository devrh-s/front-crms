...
export default function ObjectComponent({ id }: { id: number }) {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await apiGetData(`objects/${id}`);
      return response;
    },
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_objects_activities',
      };
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setEditsData({
          progressableId: object_id,
          countEdits,
          progressableType,
        })
      );
      setGuidesData({ progressableId: object_id, progressableType });
    }
    return () => {
