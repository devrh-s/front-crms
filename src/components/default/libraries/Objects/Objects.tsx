...
import ObjectsCards from './ObjectsCards';
import { IObjectsFilters } from './types';
import { useGuidesStore } from '@/zustand/guidesStore';

const ObjectsActions = dynamic(() => import('./ObjectsActions'), {
  ssr: false,
});
const ObjectsFilter = dynamic(() => import('./ObjectsFilter'), {
  ssr: false,
});
...
export default function Objects() {
  ...
  const {
    ...
    toggleFilters,
    handleSetFilters,
  } = useFilters<IObjectsFilters>(false);
  const {
    paginationModel,
    rowCount,
    handleRowCount,
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['objects', { paginationModel, sortOptions, debouncedSearchValue, applyFilters }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`objects?${searchParams}`);
      return response;
    },
  ...
  const pagePermissions = useMemo(
    () => getPagePermissions('Libraries', 'Objects', permissions),
    [permissions]
  );
  ...
  const columns: GridColDef[] = useMemo(
    () => [
        ...
        getActions: (params: { id: number; row: any }) => {
          const actions = [
            <GridActionsCellItem
              key='Action Profile'
              icon={<VisibilityIcon />}
              onClick={() => router.push(`/objects/${params.id}`)}
              label='Profile'
              showInMenu
            />,
            <GridActionsCellItem
              ...
              onClick={() => handleClickCopy(`/objects/${params.id}`)}
              label='Copy'
              showInMenu
            />,
          ];
      ...
      },
      {
        field: 'formats',
        disableColumnMenu: true,
        headerName: fields?.formats ?? 'Formats',
        description: 'Formats',
        renderCell: (params: GridRenderCellParams<any, any, any, GridTreeNodeWithRender>) => {
          const sx = {
            height: config.datagrid.rowHeight,
      ...
      {view === 'table' &&
        (mdDown ? (
          ...
          <CustomTable
            ...
            multiDeletePermission={isAdmin || !!pagePermissions['delete_objects']}
