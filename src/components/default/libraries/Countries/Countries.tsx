...
const CountryActions = dynamic(() => import('./CountryActions'), {
  ssr: false,
});
...
export default function Countries() {
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['countries', { paginationModel, sortOptions, debouncedSearchValue, applyFilters }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`countries?${searchParams}`);
      return response;
    },
    ...
    () => [
        ...
        getActions: (params: { id: number; row: any }) => {
          const actions = [
            ...
            <GridActionsCellItem
              ...
              onClick={() => handleClickCopy(`/countries/${params.id}`)}
              label='Copy'
              showInMenu
            />,
          ];
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams<any, any, any, GridTreeNodeWithRender>) => (
          <CustomChip
            handleClick={() => router.push(`/countries/${params.id}`)}
            label={params?.value}
      ...
      },
      {
        field: 'cities',
        disableColumnMenu: true,
        headerName: fields?.cities ?? 'Cities',
        description: 'Cities',
        align: 'center',
        display: 'flex',
        width: 150,
      ...
      },
      {
        field: 'image_icon',
        headerName: 'Icon',
        description: fields?.image_icon ?? 'Icon',
        disableColumnMenu: true,
        display: 'flex',
        align: 'center',
        width: 80,
      ...
      {view === 'table' &&
        (mdDown ? (
          ...
          <CustomTable
            ...
            multiDeletePermission={isAdmin || !!pagePermissions['delete_countries']}
            handlePagination={handlePagination}
            toggleFilters={toggleFilters}
            searchValue={searchValue}
            handleSearch={handleSearch}
