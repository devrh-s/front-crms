...
export default function Department({ id }: { id: number }) {
  ...
  const {
    ...
    queryKey: ['department', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`departments/${id}`);
      return response;
    },
    refetchOnWindowFocus: false,
    placeholderData: {
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_departments_activities',
      };
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setEditsData({
          progressableId: department_id,
          countEdits,
          progressableType,
        })
      );
      setGuidesData({ progressableId: department_id, progressableType });
    }
    return () => {
