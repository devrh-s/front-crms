...
export default function Position({ id }: { id: number }) {
  ...
  const {
    ...
    queryKey: ['position', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`positions/${id}`);
      return response;
    },
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_positions_activities',
      };
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setEditsData({
          progressableId: position_id,
          countEdits,
          progressableType,
        })
      );
      setGuidesData({ progressableId: position_id, progressableType });
    }
    return () => {
