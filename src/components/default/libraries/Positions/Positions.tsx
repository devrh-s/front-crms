...
const PositionActions = dynamic(() => import('./PositionActions'), {
  ssr: false,
});
...
export default function Positions() {
  ...
  const {
    ...
    toggleFilters,
    handleSetFilters,
  } = useFilters<IPositionsFilters>(false);
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['positions', { paginationModel, sortOptions, debouncedSearchValue, applyFilters }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`positions?${searchParams}`);
      return response;
    },
  ...
  const pagePermissions = useMemo(
    () => getPagePermissions('Libraries', 'Positions', permissions),
    [permissions]
  );
        ...
        getActions: (params: { id: number; row: any }) => {
          const actions = [
            <GridActionsCellItem
              key='Action Profile'
              icon={<VisibilityIcon />}
              onClick={() => router.push(`/positions/${params.id}`)}
              label='Profile'
              showInMenu
            />,
            <GridActionsCellItem
              ...
              onClick={() => handleClickCopy(`/positions/${params.id}`)}
              label='Copy'
              showInMenu
            />,
          ];
      ...
      {view === 'table' &&
        (mdDown ? (
          ...
          <CustomTable
            ...
            multiDeletePermission={isAdmin || !!pagePermissions['delete_positions']}
            toggleFilters={toggleFilters}
