...
export default function Level({ id }: { id: number }) {
  ...
  const {
    ...
    queryKey: ['level', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`levels/${id}`);
      return response;
    },
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_levels_activities',
      };
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setEditsData({
          progressableId: item_id,
          countEdits,
          progressableType,
        })
      );
      setGuidesData({ progressableId: item_id, progressableType });
    }
    return () => {
