...
export default function City({ id }: { id: number }) {
  ...
  const {
    ...
    queryKey: ['city', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`cities/${id}`);
      return response;
    },
    refetchOnWindowFocus: false,
    placeholderData: {
  ...
  useEffect(() => {
    if (tabsData) {
      ...
      const permissionTypeMap: Record<string, string> = {
        Activities: 'view_cities_activities',
      };
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setEditsData({
          progressableId: city_id,
          countEdits,
          progressableType,
        })
      );
      setGuidesData({ progressableId: city_id, progressableType });
    }
    return () => {
