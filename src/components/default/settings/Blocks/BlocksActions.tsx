...
type BlocksFields = 'name' | 'table_name' | 'entities';
...
export default function BlocksActions({
  id,
  commonData,
  visible,
  isDuplicate = false,
  ...
  const handleErrors = (errors: any, status: number) => {
    for (const [key, value] of Object.entries(errors)) {
      const errorKey = key as BlocksFields;
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiSetData('blocks', data),
    onSuccess: (result) => {
      if (result.success) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => apiUpdateData(`blocks/${id}`, data),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['blocks'],
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: ['block', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`blocks/${id}`);
  ...
  useEffect(() => {
    if (data) {
      reset({
        ...
        entities: data.entities.map((entity: { id: number; name: string }) => entity.id),
        reason: '',
      });
      setCreationInfo({
