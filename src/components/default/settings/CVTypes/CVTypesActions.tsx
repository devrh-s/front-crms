...
export default function CVTypesActions({
  id,
  visible,
  isDuplicate = false,
  handleActions,
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiSetData('cv-types', data, true),
    onSuccess: (result) => {
      if (result.success) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => apiUpdateData(`cv-types/${id}`, data, true),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['cv-types'],
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: ['cv-type', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`cv-types/${id}`);
