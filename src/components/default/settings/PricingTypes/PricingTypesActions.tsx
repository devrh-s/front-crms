...
export default function PricingTypesActions({
  id,
  visible,
  isDuplicate = false,
  handleActions,
  ...
  const handleErrors = (errors: any, status: number) => {
    for (const [key, value] of Object.entries(errors)) {
      const errorKey = key as PricingTypesFields;
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiSetData('pricing-types', data),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['pricing-types'],
        });
        hideHandler();
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => apiUpdateData(`pricing-types/${id}`, data),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['pricing-types'],
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: ['pricing-type', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`pricing-types/${id}`);
