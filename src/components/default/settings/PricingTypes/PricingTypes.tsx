...
import PricingTypesCards from './PricingTypesCards';
import { useGuidesStore } from '@/zustand/guidesStore';

const PricingTypesActions = dynamic(() => import('./PricingTypesActions'), {
  ssr: false,
});
...
export default function PricingTypes() {
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['pricing-types', { paginationModel, sortOptions, debouncedSearchValue }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`pricing-types?${searchParams}`);
      return response;
    },
    ...
    () => [
      {
        ...
        getActions: (params: { id: number }) => [
          ...
          <GridActionsCellItem
            ...
            onClick={() => handleClickCopy(`/pricing-types/${params.id}`)}
            label='Copy'
            showInMenu
          />,
          <GridActionsCellItem
