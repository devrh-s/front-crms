...
export default function ContentTypesActions({
  id,
  visible,
  isDuplicate = false,
  handleActions,
  ...
  const handleErrors = (errors: any, status: number) => {
    for (const [key, value] of Object.entries(errors)) {
      const errorKey = key as CVTypeFields;
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiSetData('content-types', data, true),
    onSuccess: (result) => {
      if (result.success) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => apiUpdateData(`content-types/${id}`, data, true),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['content-types'],
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: ['content-type', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`content-types/${id}`);
