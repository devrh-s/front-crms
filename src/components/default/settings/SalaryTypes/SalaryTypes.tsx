...
import SalaryTypesCards from './SalaryTypesCards';
import { useGuidesStore } from '@/zustand/guidesStore';

const SalaryTypesActions = dynamic(() => import('./SalaryTypesActions'), {
  ssr: false,
});
...
export default function SalaryTypes() {
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['salary-types', { paginationModel, sortOptions, debouncedSearchValue }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`salary-types?${searchParams}`);
      return response;
    },
    ...
    () => [
      {
        ...
        getActions: (params: { id: number; row: any }) => [
          ...
          <GridActionsCellItem
            ...
            onClick={() => handleClickCopy(`/salary-types/${params.id}`)}
            label='Copy'
            showInMenu
          />,
          <GridActionsCellItem
