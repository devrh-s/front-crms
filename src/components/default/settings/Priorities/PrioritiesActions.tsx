...
type PrioritiesFields = 'name' | 'color';
...
interface IPriorityActionsProps {
  id: number | null;
  visible: boolean;
  isDuplicate?: boolean;
...
export default function PrioritiesActions({
  id,
  visible,
  isDuplicate = false,
  handleActions,
}: IPriorityActionsProps) {
  ...
  const handleErrors = (errors: any, status: number) => {
    for (const [key, value] of Object.entries(errors)) {
      const errorKey = key as PrioritiesFields;
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiSetData('priorities', data),
    onSuccess: (result) => {
      if (result.success) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => apiUpdateData(`priorities/${id}`, data),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['priorities'],
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: ['priority', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`priorities/${id}`);
  ...
  return (
    ...
    >
      <TextField
        ...
        disabled={data && (data.name === 'Primary' || data.name === 'Secondary')}
        sx={{
      ...
      <Controller
        ...
        control={control}
        render={({ field }) => (
          <ColorInput field={field} error={errors.color} required label='Color' />
        )}
      />
    </ActionsDrawer>
  );
