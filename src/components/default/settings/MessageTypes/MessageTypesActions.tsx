...
type MessagesFields = 'name' | 'reason';
...
export default function MessagesActions({
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiSetData('message-types', data),
    onSuccess: (result) => {
      if (result.success) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => apiUpdateData(`message-types/${id}`, data),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['message-types'],
  ...
  const { data, isFetching, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiGetData(`message-types/${id}`);
