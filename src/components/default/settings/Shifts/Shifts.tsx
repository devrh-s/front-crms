...
const ShiftsActions = dynamic(() => import('./ShiftsActions'), {
  ssr: false,
});
...
export default function Shifts() {
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['shifts', { paginationModel, sortOptions, debouncedSearchValue }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`shifts?${searchParams}`);
      return response;
    },
    ...
    () => [
      {
        ...
        getActions: (params: { id: number }) => [
          ...
          <GridActionsCellItem
            ...
            onClick={() => handleClickCopy(`/shifts/${params.id}`)}
            label='Copy'
            showInMenu
          />,
          <GridActionsCellItem
      ...
      {
        field: 'start_time',
        headerName: fields?.start_time ?? 'Start time',
        description: 'Start time',
        disableColumnMenu: true,
        align: 'center',
        display: 'flex',
        renderCell: (params: GridRenderCellParams<any, any, any, GridTreeNodeWithRender>) =>
          params.value ? <Time time={params.value} /> : '',
        flex: 2,
      },
      {
        field: 'end_time',
        headerName: fields?.end_time ?? 'End time',
