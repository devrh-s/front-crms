...
import CommentTypesCard from './CommentTypesCard';
import { useGuidesStore } from '@/zustand/guidesStore';

const CommentTypesActions = dynamic(() => import('./CommentTypesAction'), {
  ssr: false,
});
...
export default function CommentTypes() {
  ...
  const { isFetching, isError, data } = useQuery({
    queryKey: ['comment-types', { paginationModel, sortOptions, debouncedSearchValue }],
    queryFn: async () => {
      ...
      const response = await apiGetData(`comment-types?${searchParams}`);
      return response;
    },
    ...
    () => [
      {
        ...
        getActions: (params: { id: number }) => [
          ...
          <GridActionsCellItem
            ...
            onClick={() => handleClickCopy(`/comment-types/${params.id}`)}
            label='Copy'
            showInMenu
          />,
          <GridActionsCellItem
