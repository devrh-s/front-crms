...
interface ICommentTypesActionsProps {
  id: number | null;
  visible: boolean;
  isDuplicate?: boolean;
...
export default function CommentTypesActions({
  id,
  visible,
  isDuplicate = false,
  handleActions,
}: ICommentTypesActionsProps) {
  ...
  const handleErrors = (errors: any, status: number) => {
    for (const [key, value] of Object.entries(errors)) {
      const errorKey = key as CommentTypesFields;
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiSetData('comment-types', data),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['comment-types'],
        });
        hideHandler();
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) => apiUpdateData(`comment-types/${id}`, data),
    onSuccess: (result) => {
      if (result.success) {
        queryClient.invalidateQueries({
          queryKey: ['comment-types'],
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: ['comment-types', id ?? null],
    queryFn: async () => {
      const response = await apiGetData(`comment-types/${id}`);
